
	</main>
	<footer class="footer">
		<div id="page-top"></div>
		<div class="fter_menu clearfix">
			<div class="ftr_logo"><img src="<?php echo get_template_directory_uri() ?>/_old_assets/img/enlogo.png" alt="アルトリスト株式会社"></div>
			<div class="ftr_left ">
				<div class="ftr_txt">
					〒182-0026<br />
					東京都調布市小島町1-1-1<br />
					電気通信大学「UECアライアンスセンター510号室」<br />
					TEL 042-444-2477 / FAX 042-444-2478
				</div>
				<div class="sns_btn">
					<a href="https://www.youtube.com/@altruist640" target="_blank"><img src="<?php echo get_template_directory_uri() ?>/_old_assets/img/btn_youtube.png" alt="YouTube"></a>
				</div>
			</div>

			<div class="ftr_right sp_none">
				<div class="ftr_link"><a href="./company.php">企業情報</a></div>
				<div class="ftr_link">
					<a href="./equipment.php">FA技術営業</a>　　<a href="./engineering.php">プラントエンジニアリング</a>　　<a href="./robots.php">技術（相模原技術センター）</a>
				</div>
				<div class="ftr_link"><a href="./products.php">製品情報</a></div>
				<div class="ftr_link"><a href="https://en-gage.net/altruist_saiyo/" target="_blank">採用情報 <img class="nav01_icon" src="<?php echo get_template_directory_uri() ?>/_old_assets/img/gaibu_link.png" /></a></div>
			</div>
		</div>
		<div class="ftr_copy"><span class="sp_none"><a href="./terms.php">サイト利用規約</a><span>｜</span><a href="./privacy.php">プライバシーポリシー</a><span>｜</span><a href="./contact.php">お問い合わせ</a><br /></span>&copy; Altruist Co., Ltd. All Rights Reserved.</div>

<!--下部バナー-->

  <div class="cookie-consent">
    <div class="cookie-text">当ウェブサイトでは、お客様により良いサービスを提供するため、cookieを利用しています。cookieの使用に関する詳細は「<a href="./privacy.php">プライバシーポリシー</a>」をご覧ください。</div>
    <div class="cookie-agree">OK</div>
  </div>

<!--下部バナここまでー-->


	</footer>